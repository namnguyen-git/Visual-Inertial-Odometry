...
public:
    ...
    vector<DMatch> rejectStereoOutliers(vector<KeyPoint>& keyPoints1, vector<KeyPoint>& keyPoints2, vector<DMatch>& matches, const Matrix3f& F);

    void rejectStereoOutliers(vector<Point2f>& feature1, vector<Point2f>& feature2, vector<uchar>& status, const Matrix3f& fundMatrix);

    void rejectStereoOutliers(vector<Point2f>& feature1, vector<uchar>& status1, vector<Point2f>& feature2, vector<uchar>& status2, vector<uchar>& status, const Matrix3f& fundMatrix);
    ...
    vector<Vector3f> computeStereoFeaturePose(vector<KeyPoint> &keyPoints1, vector<KeyPoint> &keyPoints2, vector<DMatch> &stereoMatches, Matrix3f& K1, Matrix3f& K2, MatrixXf& D1, MatrixXf& D2, Matrix3f& R, Vector3f& T, vector<float>& allQuality);

    Vector3f computeCameraPose(vector<Feature>& pointCloud, Vector3f& preCameraPose, Matrix3f R, int thresInliers, vector<float> trackError);
    ...
    vector<Vector3f> convertKeypointsToObservation(const vector<KeyPoint>& keyPoints, vector<DMatch> stereoMatches, bool primary, Matrix3f& K, MatrixXf& D);

    vector<Vector3f> convert2DFeaturesToObservation(const vector<Point2f> features, const Matrix3f& K, const MatrixXf& D);

    vector<Point2f> extractMatchedFeatures(vector<KeyPoint> keyPoints, vector<DMatch> matches, bool primary);

    vector<Point2f> convertKeyPointToPoint2f(vector<KeyPoint>& keyP);

    vector<KeyPoint> convertPoint2fToKeyPoint(vector<Point2f>& p);
