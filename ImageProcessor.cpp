...
vector<DMatch> ImageProcessor::rejectStereoOutliers(vector<KeyPoint>& keyPoints1, vector<KeyPoint>& keyPoints2, vector<DMatch>& matches, const Matrix3f& fundMatrix)
{
...
void ImageProcessor::rejectStereoOutliers(vector<Point2f>& feature1, vector<Point2f>& feature2, vector<uchar>& status, const Matrix3f& fundMatrix)
{
    Vector3f v1, v2;
    float result;
    int N = feature1.size();
...
void ImageProcessor::rejectStereoOutliers(vector<Point2f>& feature1, vector<uchar>& status1, vector<Point2f>& feature2, vector<uchar>& status2, vector<uchar>& status, const Matrix3f& fundMatrix)
{
    Vector3f v1, v2;
    float result;
    int N = feature1.size();
...
vector<Vector3f> ImageProcessor::computeStereoFeaturePose(vector<KeyPoint> &keyPoints1, vector<KeyPoint> &keyPoints2, vector<DMatch> &stereoMatches, Matrix3f& K1, Matrix3f& K2, MatrixXf& D1, MatrixXf& D2, Matrix3f& R, Vector3f& T, vector<float>& allQuality)
{
...
Vector3f ImageProcessor::computeCameraPose(vector<Feature>& pointCloud, Vector3f& preCameraPose, Matrix3f R, int thresInliers, vector<float> trackError)
{
...
vector<Vector3f> ImageProcessor::convertKeypointsToObservation(const vector<KeyPoint>& keyPoints, vector<DMatch> matches, bool primary, Matrix3f& K, MatrixXf& D)
{
...
vector<Point2f> ImageProcessor::extractMatchedFeatures(vector<KeyPoint> keyPoints, vector<DMatch> matches, bool primary)
{
...
vector<Point2f> ImageProcessor::convertKeyPointToPoint2f(vector<KeyPoint>& keyP)
{
    vector<Point2f> p;
    int size = keyP.size();
    for (int i = 0; i < size; i++){
...
vector<KeyPoint> ImageProcessor::convertPoint2fToKeyPoint(vector<Point2f>& p)
{
    vector<KeyPoint> keyP;
    Point2f tmp;
